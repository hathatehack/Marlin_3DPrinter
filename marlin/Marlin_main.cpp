...
void process_commands(char* cmd=cmdbuffer[bufindr]); //By default we fetch command from "cmdbuffer", BUT the "cmd" should point to "cmdsd" called by manage_sdprint When FASTLOGGING because
                                                     //we don't want to mess up commands from uart and sdcard(we don't buffer it, just fetch and process right away)!
...
void loop()
{
  ...
  if(buflen)
  {
      ...
      if(card.saving) {
        ...
        {
          if((card.getfilesize() >= 1024*10))//&& ((starttime + 5000) < millis())) //Wait until we reach a certain level!
          {
            card.fastlogging = 2;
...
//              MSerial.print(cmdbuffer[bufindw]);
  ...
  #ifdef SDSUPPORT
  #if FASTLOG
  if(card.saving || !card.sdprinting || serial_count!=0 || buflen >= BUFSIZE) {
  #else
...
static char* cmd_pointer;
char* code_seen(char code)
{
  strchr_pointer = strchr(cmd_pointer, code);//strchr(cmdbuffer[bufindr], code);
